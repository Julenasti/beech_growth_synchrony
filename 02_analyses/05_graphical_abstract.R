...
prec_norm <- normalize(conclima$prec)

prec_norm_year <- prec_norm / 12
...
ggplot(conclima, aes(year, mean_syn,
                     color = type)) +
  geom_col(
    aes(y = prec_norm_year, x = year),
    color = alpha("dodgerblue2", 0.1),
    fill = alpha("dodgerblue2", 1),
    size = 2,
  ) +
  ...
  scale_y_continuous(
    ...
    sec.axis = sec_axis(~ . * (max(conclima$prec) - min(conclima$prec)) + min(conclima$prec),
